...
| **注文** | `/store/orders` | クイックタイル「注文」 |
| 顧客 / 分析 / 設定 | /store 外の別モジュール | Phase 0 はナビのみ（プレースホルダ） |

**Figma（fileKey `mXIeaaZlPI1oRmjaIKgnHh` / ページ「store画面」852:338）— 全画面作成済み（2026-06-11）:**

| 画面 | node | 位置 |
| --- | --- | --- |
| ダッシュボード | `876:337` | x3200 |
| 商品一覧（テーブル/CRUD） | `952:337` | x6320 |
| 商品編集（フォーム+プレビュー+チェックリスト） | `952:540` | x7880 |
| 注文一覧 | `952:743` | x9440 |
| クイック作成ダイアログ | `960:337` | x11000 |
| （参考・不採用）地色 B 案比較 | `932:337` | x4760 |
...
**Hero と「最近の注文」セクションは置かない**（Figma 更新済み・フレーム高 1217）。
ストア名/URL は chrome ヘッダーが持ち、注文はサイドバー「注文」とクイックタイルから到達する。
...
- 入口は 2 つ（サイドバー「注文」/ ダッシュボードのクイックタイル「注文」）ですべてこの画面に着地する（§1）。
  ダッシュボードに注文セクションは置かない（2026-06-11 決定・Figma 反映済み）。
