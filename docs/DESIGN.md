...
| 塗りチップ | Badge（bold） | 分類メタデータ | デジタル=`green` / コース=`indigo` / 予約=`orange` / サブスク=`purple`。`yellow` は割引専用に予約。blue チップは accent と紛らわしいため使わない |
...
- 形が違えば同系色の併存は許容（例: デジタル緑チップとテーブルの緑ドット公開中）。
