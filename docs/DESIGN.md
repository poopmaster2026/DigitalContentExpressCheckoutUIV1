...
3. **色分けは「色がそれ自体常時必要な情報」の場合のみ** — 写真等のコンテンツが
   ...
   カードのメタデータは公式 AssetCard と同じ **プレーンテキスト 1 行**
   （例: `レシピ本・¥1,200`、区切りは「・」）。Badge を使うなら例外状態（下書き・
   エラー等）だけ。non-semantic 色分け（8 色以内、blue は accent と紛らわしいため
   回避、yellow は割引/無料専用）は、色が常時意味を持つ画面に限る。
...
spectrum.adobe.com の Badge 仕様（`docs/spectrum/components.md` の Badge 節）に基づく。
**ステータスは semantic、販売形態は non-semantic** で、互いに色を被らせない。

| 用途 | variant | 根拠 |
|---|---|---|
| 公開中（状態） | `informative`（青） | Badge semantic「Informative=公開」 |
| 下書き（状態） | `neutral`（灰） | Badge semantic「Neutral=下書き」 |
| 完了系（将来） | `positive`（緑） | Badge semantic「Positive=完了」 |
| 失敗/エラー（将来） | `negative`（赤） | Badge semantic「Negative=失敗」 |
| デジタル（販売形態） | `green` | non-semantic 8 色内。販売形態選択 UI の配色 |
| コース（販売形態） | `indigo` | blue は accent/informative と紛らわしいため indigo で代替 |
| 予約（販売形態） | `orange` | 販売形態選択 UI の配色 |
| サブスク（販売形態） | `purple` | 販売形態選択 UI の配色 |
| 無料/割引 | `yellow` | docs 上 yellow は「割引」専用（予約済み・他用途に使わない） |

濃さは公式 Gallery 例（yellow Free バッジ）と同じ **bold（既定）**。
新しい色を足すときは semantic（informative/neutral/positive/negative/yellow）と
被らない hue を選ぶこと。
