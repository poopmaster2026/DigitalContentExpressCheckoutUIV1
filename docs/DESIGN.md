...
| 塗りチップ | Badge（bold） | 分類メタデータ | デジタル=`indigo` / コース=`cyan` / 予約=`magenta` / サブスク=`purple`（最頻出のデジタルを accent 同系の indigo に寄せる）。`yellow` は割引専用に予約。blue チップは accent と紛らわしいため、緑系はプロダクト方針により使わない |
