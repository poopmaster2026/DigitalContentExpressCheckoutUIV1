...
主要 CTA       → Button variant="accent"  (--accent: #007AFF Apple blue)
...
AppShell:
  ...
  │   ├── + 作成ボタン（accent #007AFF, borderRadius: full）
  │   └── ナビ: Home / 商品 / 注文 / 分析 / 設定
...
| 症状 | このプロジェクトでの正解 |
|---|---|
| 全体が「白背景＋青いアクセント」 | キャンバス `gray-75`、ヘッダー `#1d1d1d`、accent は `#007AFF`（青はアクセントのみ・面に使わない） |
