...
- Build custom components with React Aria Components for behavior and accessibility, and the S2 `style` macro for Spectrum styling. See [Creating Custom Components](creating-custom-components.md) for the full pattern (`focusRing`, `baseColor`, `pressScale`, render props, animations, etc.).
