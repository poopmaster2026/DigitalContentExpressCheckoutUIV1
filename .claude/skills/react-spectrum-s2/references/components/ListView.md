...
<ListView styles={style({ width: "full", maxWidth: 400, height: 320 })}>
...
let images = [
  ...
  {
    id: "mountain-with-lake",
    name: "Mountain with lake.png",
    image_url:
      "https://images.unsplash.com/photo-1722172118908-1a97c312ce8c?q=80&w=600&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  },
];
...
function AsyncListView() {
  ...
  return (
    <ListView
      ...
      styles={style({ width: "full", maxWidth: 400, height: 320 })}
    >
      {(item) => <ListViewItem id={item.name}>{item.name}</ListViewItem>}
    </ListView>
  );
}
```
...
<ListView
  ...
  renderEmptyState={() => (
    <IllustratedMessage>
      ...
      <Content>
        Press <Link href="https://adobe.com">here</Link> for more info.
      </Content>
    </IllustratedMessage>
  )}
>
...
import {
  ...
  ListViewItem,
  type Selection,
} from "@react-spectrum/s2/ListView";
        ...
        renderActionBar={(selectedKeys) => {
          ...
          return (
            <ActionBar>
              <ActionButton
                aria-label="Edit"
                onPress={() => alert(`Edit ${selection}`)}
              >
                <Edit />
              </ActionButton>
              <ActionButton
                aria-label="Copy"
                onPress={() => alert(`Copy ${selection}`)}
              >
                <Copy />
              </ActionButton>
              <ActionButton
                aria-label="Delete"
                onPress={() => alert(`Delete ${selection}`)}
              >
                <Delete />
              </ActionButton>
            </ActionBar>
      ...
      >
        {(item) => (
          <ListViewItem
            id={item.id}
            textValue={item.name}
            hasChildItems={item && "children" in item}
          >
            {item.icon}
            <Text>{item.name}</Text>
          </ListViewItem>
